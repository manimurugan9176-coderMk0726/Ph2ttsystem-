// =========================================================
// MediTrack Pro - Engine with Duty Roster & Live Clinical Alerts
// =========================================================

let currentViewRole = "Doctor"; // 'Doctor' or 'Nurse'
let nurseFilterMode = "all";   // 'all' or 'mine'

// Initial Seed Patient Records with Assigned Nurses & Doctors
const seedRecords = [
  {
    id: 101,
    name: "Kavitha Raj",
    ...
    history: [
      {
        ...
        rx: "Metformin 500mg BD continued. Blood glucose log updated.",
        notes: "FBS stable. Nurse to check post-meal sugar daily."
      }
    ]
  },
  {
    ...
    bed: "ICU - Bed 02",
    assignedDoctor: "Dr. Arvind (Cardiology)",
    assignedNurse: "Nurse Anita (ICU)",
    diagnosis: "Acute Hypertension & Angina",
    vitals: { bp: "175/115", pulse: "108", spo2: "93%" }, // Triggers Emergency Alert
    status: "Critical",
    ...
    history: [
      {
        date: "2026-09-12",
        staff: "Dr. Arvind (Cardiology)",
        rx: "Emergency IV Nitroglycerin infusion running. Continuous arterial line check.",
        notes: "Nurse Anita to monitor vitals every 30 minutes."
      }
    ]
  },
  {
...
// --- Live Real-Time Clinical Alert Engine ---
function checkClinicalAlerts() {
  const alertBar = document.getElementById("alertBar");
  const alertMsg = document.getElementById("alertMessage");

  // Filter critical conditions or abnormal vitals
  ...
  if (criticalPatients.length > 0) {
    ...
    alertMsg.innerHTML = `<strong>EMERGENCY ALERT:</strong> Attention Required for: <span style="color:#fff;">${alertNames}</span>`;
    ...
    alertMsg.innerHTML = `<strong>NORMAL STATUS:</strong> All vitals stable. Attending: <strong>${activeDoc}</strong> | Ward Nurse: <strong>${activeNur}</strong>`;
...
function createPatient(e) {
    ...
    history: [
      {
        ...
        notes: `Admitted under ${assignedNurse} supervision.`
      }
    ]
  };
  ...
  syncData();
  e.target.reset();
  alert("Patient File Created and Assigned to Nursing Roster!");
}
...
function appendRx(id) {
  ...
  const newRx = prompt(`Enter Medication / Nursing Order for ${p.name}:`);
  if (!newRx || !newRx.trim()) return;

  const notes = prompt("Enter Clinical Progress Notes / Response:") || "Routine round observation";
...
function printSlip(id) {
  ...
  win.document.write(`
    <html>
      <head>
        <title>Prescription & Nursing Chart - ${p.name}</title>
        <style>
      ...
      <body>
        ...
        <table class="info-table">
          ...
          <tr>
            <td colspan="3"><strong>Latest Recorded Vitals:</strong> BP: ${p.vitals?.bp || "N/A"} | Pulse: ${p.vitals?.pulse || "N/A"} bpm | SpO2: ${p.vitals?.spo2 || "N/A"}</td>
          </tr>
        </table>
        <div class="rx-box">
          ...
          <p><strong>Clinical / Nursing Notes:</strong> ${latest.notes}</p>
        </div>
        <div class="footer">
...
function render(list) {
  ...
  if (list.length === 0) {
    box.innerHTML = '<div style="text-align:center; padding:35px; color:var(--text-muted);">No matching clinical files found in this category.</div>';
    return;
  }
  ...
  list.forEach(p => {
    ...
    card.innerHTML = `
      ...
      <!-- Doctor and Assigned Nurse Badges -->
...
document.addEventListener("DOMContentLoaded", () => {
  refresh();
  // Auto-alert check every 25 seconds
  setInterval(checkClinicalAlerts, 25000);
});
